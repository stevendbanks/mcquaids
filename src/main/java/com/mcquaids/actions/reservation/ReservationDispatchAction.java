        ...
        try {
            ...
            for (DispatchAction action : existingActions) {
                if (removedEquipment.contains(action.getEquipmentNumber())) {
                    action.setRemovedFromReservation(true);
            ...
            ctx.put("sourceType", "RESERVATION");
            ctx.put("sourceId", reservationID);

            ctx.put("dispatchPlanStatusText", reservation.getReservationStatusText());

            // Use real timestamps if available, otherwise fallback
            ctx.put("createdDateTime", reservation.getDateCreated());
